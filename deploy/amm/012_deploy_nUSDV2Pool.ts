...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (nUSDPoolV2) {
    ...
    if (await getChainId() === CHAIN_ID.OPTIMISM_KOVAN) {
      TOKEN_ADDRESSES = [
        (await get("nUSD")).address,
        (await get("DAI")).address,
        (await get("USDC")).address,
    ...
    if (currentOwner == deployer) {
      log(
        `transferring the ownership of "nUSDPoolV2" to the multisig: ${(await get("DevMultisig")).address}`,
      )
      await execute(
        "nUSDPoolV2",
        { from: deployer, log: true },
      ...
      log(
        `"nUSDPoolV2" is already owned by the multisig: ${(await get("DevMultisig")).address}`,
      )
    }
